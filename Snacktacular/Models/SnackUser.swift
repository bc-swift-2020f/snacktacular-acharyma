...
class SnackUser {
    ...
    convenience init(dictionary: [String: Any]) {
        ...
        let timeIntervalData = dictionary["userSince"] as! TimeInterval? ?? TimeInterval()
        let userSince = Date(timeIntervalSince1970: timeIntervalData)
    ...
    func saveIfNewUser(completion: @escaping (Bool) -> ()) {
        ...
        userRef.getDocument { (document, error) in
            guard error == nil else {
                print("ERROR: coild'nt access document for user \(self.documentID)")
                return completion(false)
            }
            guard document?.exists == false else {
                print("The document for user \(self.documentID) already exists. No need to create it!")
